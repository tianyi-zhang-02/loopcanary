...
* ``clean_pass_rate`` — fraction of trajectories with
  :attr:`~loopcanary.core.verdict.IntegrityLabel.CLEAN`.
* ``non_exploit_pass_rate`` — pass rate after stripping only
  trajectories labeled
  :attr:`~loopcanary.core.verdict.IntegrityLabel.EXPLOIT`.
...
class ComparisonReport(BaseModel):
    """Output of ``loopcanary compare`` — two submissions side-by-side."""
