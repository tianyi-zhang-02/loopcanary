# loopcanary status report (PR #1 readiness review)

Read-only diagnostic, 2026-05-27. ~45 min of diff/doc review. Written to
inform the merge decision on
[PR #1](https://github.com/tianyi-zhang-02/loopcanary/pull/1). No code
...
| Type-check | `mypy src/loopcanary` — no issues, 38 files |
...
**Task 4 — CLI**
- `4f0a956 feat(cli): add loopcanary run command`
...
| MALT ingestion — live HF download + load + filter | **working** (verified end-to-end in Phase 3, loaded 20 manually-reviewed trajectories balanced 10/10) | manual run via `loopcanary run …` |
...
| `loopcanary run` CLI — flags + validation + JSON output | **working** (hermetic) | `tests/test_cli.py` (9 tests) |
...
`src/loopcanary/report.py:238-269`. The clean baseline builds two parallel
numpy arrays:
...
|---|---|---|
| `src/loopcanary/cli.py:212-220` (monitor exception path) | `monitor_score=None, abstain=True` | ✓ |
| `src/loopcanary/cli.py:227-235` (verdict path) | `score = None if verdict.abstain else _verdict_to_score(...)`, `abstain=verdict.abstain` | ✓ |
...
`src/loopcanary/monitors/metr_prompt.py:99-114`. `_format_transcript` emits
...
`src/loopcanary/adapters/malt.py:339-341`. The `_md(...) or _md(...) or ...
...
`src/loopcanary/adapters/malt.py:199-205`. The framing-skip logic only
...
`src/loopcanary/core/trajectory.py` (pre-existing). When an
...
`src/loopcanary/transformations/structural.py:158`. `apply_structural_battery`
...
| `docs/related-work.md` | **STALE — frames loopcanary as the pre-pivot thing** | Positions the project against EvilGenie/RHA/TRACE as a "composed multi-layer framework with structured AuditVerdict." Not what v0.1 ships. |
...
- `related-work.md` claims loopcanary's "contribution is to compose
...
1. **Merge PR #1 with two pre-merge fixes** — *medium effort,
   ...
   for what loopcanary actually is." ~30-45 min total. **Without this,
   the outside-view risk in §7 is real.**

2. **Resolve Q5 (live integration) post-merge, locally** — *low effort
   if HF + API key are in hand, ~20 min.* Run `loopcanary run --limit 20
