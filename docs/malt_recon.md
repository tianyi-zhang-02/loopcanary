...
the kind of evaluation work loopcanary is considering, with two binding
...
`src/loopcanary/adapters/malt.py` is fully implemented and is exercised by
...
`HF_TOKEN` in the shell that runs `loopcanary run`.
